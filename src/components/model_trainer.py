...
from src.utils.model_utils import load_yaml, load_csv, save_pickle
from src.utils.model_utils import create_sequences_for_regression
    ...
    def _get_model_configs(self) -> Dict[str, ModelConfig]:
        ...
        return {
            'random_forest': ModelConfig(
                ...
                hyperparameters={'n_estimators': 100, 'random_state': self.config.random_state, 'n_jobs': self.config.n_jobs},
            ...
            'linear_regression': ModelConfig(
                ...
                hyperparameters={'n_jobs': self.config.n_jobs}
            ),
            'ridge': ModelConfig(
                name='Ridge',
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            for model_name, config in self.model_configs.items():
                ...
                logger.info(f"{model_name} training completed. MSE: {metrics['mse']:.4f}, R²: {metrics['r2']:.4f}")
...
class LSTMModelTrainer(BaseModelTrainer):
    ...
    def build_lstm_model(self, input_shape: Tuple[int, int]) -> tf.keras.Model:
        ...
        try:
            ...
            # Dense layers
            x = Dense(64, activation='relu', kernel_regularizer=l2(self.lstm_config.l2_reg), name='dense_1')(x)
            x = Dropout(self.lstm_config.dropout, name='dropout_dense_1')(x)
            x = Dense(32, activation='relu', kernel_regularizer=l2(self.lstm_config.l2_reg), name='dense_2')(x)
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series) -> Tuple[tf.keras.Model, Dict[str, Any]]:
...
class ModelTrainer:
    ...
    def _load_config(self, config_path: str) -> ModelTrainerConfig:
        ...
        try:
            ...
            return ModelTrainerConfig(
                ...
                enable_hyperparameter_tuning=config_dict.get('model', {}).get('enable_hyperparameter_tuning', False),
                save_training_metrics=config_dict.get('model', {}).get('save_training_metrics', True)
            )
        except Exception as e:
            logger.error(f"Error loading model trainer config: {e}")
            raise e
    
    def _save_training_metrics(self, metrics: Dict[str, Any], filename: str = "training_metrics.json") -> None:
    ...
    def train_all_models(self, X_train: pd.DataFrame, y_train: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"All model training completed successfully. Trained {len(all_trained_models)} models.")
            return {
